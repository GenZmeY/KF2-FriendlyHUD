...
{
	multini --set "$KFEditorConf" 'ModPackages' 'ModPackages' 'GzFriendlyHUD'
	multini --set "$KFEditorConf" 'ModPackages' 'ModPackagesInPath' "$(unixpath2win "$MutSource")"
}

function compiled ()
{
	test -f "$MutStructScript/GzFriendlyHUD.u"
}
...
function compile ()
{
	...
	cp -f "$MutSource/GzFriendlyHUD/assets"/*.upk "$MutStructPackages"
...
Difficulty=\"0\"
GameLength=\"0\"
Mutators=\"GzFriendlyHUD.FriendlyHUDMutator\"
Args=\"\"" > "$MutTestingIni"
}

...
function Initialized()
{
    ...
    else
    {
        if (INIVersion < 2)
        {
            ...
            SortStrategy = 0;
            SelfSortStrategy = 1;
            DynamicOpacity = class'GzFriendlyHUD.FriendlyHUDHelper'.static.MakeCubicInterpCurve(1.f, 1.f, 4.f, 0.f);
            Opacity = 1.f;
            IconSize = 32.f;
            IconOffset = 0.f;
            IconGap = 4.f;
            ...
            OldBGColor = class'GzFriendlyHUD.FriendlyHUDHelper'.static.ColorFromString(BGColor);
            ...
            NameColor = class'GzFriendlyHUD.FriendlyHUDHelper'.static.ColorFromString(TextColor);
...
exec function SetFHUDArmorBlockProportions(float Width, optional string Ratios)
{
    ...
    for (I = 0; I < Params.Length; I++)
    {
        ...
        if (Params.Length > 1)
        {
            ...
            if (Ratio < class'GzFriendlyHUD.FriendlyHUDInteraction'.const.FLOAT_EPSILON) continue;
...
{
    ArmorBlockOutline = class'GzFriendlyHUD.FriendlyHUDHelper'.static.MakeOutline(Top, Right, Bottom, Left);
    SaveAndUpdate();
}
...
{
    HealthBlockOutline = class'GzFriendlyHUD.FriendlyHUDHelper'.static.MakeOutline(Top, Right, Bottom, Left);
    SaveAndUpdate();
}
...
{
    DynamicOpacity = class'GzFriendlyHUD.FriendlyHUDHelper'.static.MakeCubicInterpCurve(P0, P1, T0, T1);
    SaveAndUpdate();
}
